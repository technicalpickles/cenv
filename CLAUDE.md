...
- **`internal/claudeauth`** — asks `claude` itself instead of reading files, for `cenv auth status`. `Status` wraps `claude auth status` (exit code = something is stored; it never checks expiry or the network). `Probe` (also driven across envs by `cenv auth refresh`) makes one minimal real request with `ProbeArgs` (`--safe-mode`, haiku, no tools, no session persistence) to prove the credentials still work. Never swap `--safe-mode` for `--bare`: `--bare` skips keychain reads, so OAuth can't work. Shells out via a `Runner` interface like `internal/keychain`.
